...
def validate_and_regenerate_json(file_text):
    def sizeSplitter(combined_code):
        if len(combined_code) < groq_limit:
            ...
            socketio.emit('progress', {'message': 'summary_description built'})
            summary_layers = groq_summarize_text_layers(combined_code)
            socketio.emit('progress', {'message': 'summary_layers built'})
            print('under groq_limit summary_layers: ', summary_layers)
            print('under grok_limit summary_description: ', summary_description)
    ...
    summary_description, summary_content = sizeSplitter(file_text)

    summary_content = remove_content_key(summary_content)

    if not validate_json(json.dumps(summary_description)) or not isinstance(summary_content, list):
        summary_description, summary_content = sizeSplitter(file_text)
        summary_content = remove_content_key(summary_content)

    return summary_description, summary_content
...
def projectFileParser():
    ...
    try:
        summary_description, summary_content = validate_and_regenerate_json(file_text)
        return jsonify({'summary_description': summary_description, 'summary_content': summary_content}), 200
    except Exception as e:
